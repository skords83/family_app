...
function isStale(fetchedAt?: string, maxAgeMs = 10 * 60 * 1000): boolean {
...
function groupEventsByDay(
  ...
  for (const event of events) {
    ...
    // Vergangene Termine heute noch anzeigen (aber nicht überfahrene timed events)
    if (dateStr === todayStr && !event.allDay) {
      const endDate = new Date(event.end);
      if (endDate < now) continue; // bereits vorbei → überspringen
    }
  ...
  // Innerhalb jedes Tages sortieren: Ganztags zuerst, dann nach Uhrzeit
...
export default function CalendarWidget({ events = [], fetched_at, loading, daysAhead = 7 }: CalendarWidgetProps) {
  ...
  return (
    // flex-1 + min-h-0: Widget füllt die Spalte und scrollt intern
    <div
          ...
          <div className="space-y-4">
            {days.map(day => {
              ...
              return (
                <div key={day}>
                  <p
                    className="text-[10px] font-sans font-semibold uppercase tracking-wider mb-2"
                    style={{ color: '#a09d99' }}
                  >
