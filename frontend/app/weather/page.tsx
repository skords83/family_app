...
function TodayBlock({ data, todayDaily }: { data: WeatherData; todayDaily?: WeatherDaily }) {
  ...
  return (
    <div style={{ ...card, position: 'relative', overflow: 'hidden', flexShrink: 0, padding: '28px 36px' }}>
      ...
      <i
        className={`ti ${getWeatherIcon(data.weathercode).icon}`}
        aria-hidden="true"
        style={{
          position: 'absolute', right: 24, top: '50%', transform: 'translateY(-50%)',
          fontSize: 112, lineHeight: 1, opacity: 0.08, color: getWeatherIcon(data.weathercode).color,
          ...
          return (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
              ...
              <div style={{
                ...
                <i className={`ti ${getWeatherIcon(h.weathercode).icon}`} aria-hidden="true" style={{ fontSize: 22, lineHeight: 1, color: getWeatherIcon(h.weathercode).color }} />
        ...
        {daily.map((day) => {
          ...
          return (
            ...
            >
              ...
              <i className={`ti ${getWeatherIcon(day.weathercode).icon}`} aria-hidden="true" style={{ fontSize: 26, textAlign: 'center', lineHeight: 1, color: getWeatherIcon(day.weathercode).color }} />
